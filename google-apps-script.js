// ═══════════════════════════════════════════════════════════════════════════
// SIGC — Inquérito Multi-Centro · Google Apps Script v3
...
function doGet(e) {
  ...
  try {
    ...
    if (action === 'getFaixas')     return ok(getSheet('FaixasEtarias', faixasCols()).map(r => ({ ...rowToObj(r), activo: r.activo === 'true' || r.activo === true, ordem: Number(r.ordem) })))
    if (action === 'getSeccoes')    return ok(getSheet('Seccoes', secCols()).map(r => ({ ...rowToObj(r), centro_id: r.centro_id || null, ordem: Number(r.ordem) })))
    if (action === 'getPerguntas')  return ok(getSheet('Perguntas', pergCols()).map(r => ({ ...rowToObj(r), centro_id: r.centro_id || null, opcoes: (r.opcoes || '').split('|').filter(Boolean), min_chars: Number(r.min_chars) || 0, obrigatoria: r.obrigatoria === 'true' || r.obrigatoria === true, ordem: Number(r.ordem), placeholder: r.placeholder || '' })))
...
function doPost(e) {
  try {
    ...
    if (action === 'saveSeccao')               { upsert('Seccoes', secCols(), { ...data, centro_id: data.centro_id || '', subtitulo: data.subtitulo || '' }); return ok({ success: true }) }
    if (action === 'deleteSeccao')             { deleteRow('Seccoes', data.id); return ok({ success: true }) }
    if (action === 'savePergunta')             { upsert('Perguntas', pergCols(), { ...data, centro_id: data.centro_id || '', opcoes: (data.opcoes || []).join('|'), placeholder: data.placeholder || '' }); return ok({ success: true }) }
...
function upsert(shName, cols, data) {
  ...
  const allRows = sh.getLastRow() > 1 ? sh.getRange(2,1,sh.getLastRow()-1,1).getValues().flat() : []
  const idx = allRows.findIndex(id => String(id) === String(data.id))
...
function checkDevice(deviceId) {
  ...
  return sh.getRange(2,1,sh.getLastRow()-1,1).getValues().flat().some(id => String(id) === String(deviceId))
}

function saveResposta(data) {
  if (checkDevice(data.device_id)) return
  const sh = ensureSheet('Respostas', ['id','timestamp','centro_id','catequistas_ids','etapa','faixa_etaria_id','device_id','respostas_json'])
  // Suporte legado: catequista_id → catequistas_ids
  const cats = data.catequistas_ids ? (Array.isArray(data.catequistas_ids) ? data.catequistas_ids.join(',') : data.catequistas_ids) : (data.catequista_id || '')
  sh.appendRow([data.id, data.timestamp, data.centro_id, cats, data.etapa, data.faixa_etaria_id, data.device_id, JSON.stringify(data.respostas || {})])
...
function getRespostas() {
  ...
  return rows.map(r => {
    ...
    // catequistas_ids: string 'a,b,c' → array
    obj['catequistas_ids'] = obj['catequistas_ids'] ? String(obj['catequistas_ids']).split(',').filter(Boolean) : []
    try { obj['respostas'] = JSON.parse(obj['respostas_json'] || '{}') } catch { obj['respostas'] = {} }
    delete obj['respostas_json']; delete obj['device_id']
    return obj
  }).filter(r => r.id)
}
...
function savePergDesact(centro_id, pergunta_ids) {
  ...
  if (sh.getLastRow() > 1) {
    ...
    for (let i = data.length-1; i >= 0; i--) {
      if (String(data[i]) === String(centro_id)) sh.deleteRow(i+2)
    }
  }
